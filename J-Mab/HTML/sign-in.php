...
<head>
  ...
  <link rel="icon" type="image/x-icon" href="/J-Mab/imahe/jmab no bg.png">
</head>
...
<body>
  ...
  document.getElementById('signin-form').addEventListener('submit', function(event) {
    ...
    .then(data => {
        ...
        if (data.success) {
            const role = data.user ? data.user.roles : null; 
            const userId = data.user ? data.user.id : null; // Get user ID from response
            
            if (data.token && userId) {
                // Save the token & user ID to localStorage
                localStorage.setItem('authToken', data.token);
                localStorage.setItem('userId', userId); // Save user ID
                console.log('User ID & Token saved:', userId, data.token);
            } else {
            ...
            const errorMessage = data.errors ? data.errors[0] : "Login failed.";
            alert(errorMessage);
        }
    })
    .catch(error => {
